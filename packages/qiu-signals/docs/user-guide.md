...
`qiu-signals` has two layers: axes, which fix where a function is sampled, and signals, which are functions on an axis. Each lives in its own modules:

| module                                                     | contents                                                                 |
| ---------------------------------------------------------- | ------------------------------------------------------------------------ |
| [`integer_axis`][qiu_signals.integer_axis]              | `IndexOrdering`, `IntegerAxis`: samples labeled by integer indices        |
| [`physical_axis`][qiu_signals.physical_axis]            | `AxisDomain`, `PhysicalAxis` and its subclasses, `reciprocal_period`     |
| [`signal`][qiu_signals.signal]                          | `Signal`: a signal given by its sampled values                           |
| [`algebraic_signal`][qiu_signals.algebraic_signal]      | `AlgebraicSignal`, `PolynomialSignal`, `QuadraticSignal`, `SampledSignal` |
| [`arithmetic`][qiu_signals.arithmetic]                  | the arithmetic operators shared by the signal classes                    |

The package has no top-level exports; import from the modules, e.g. `from qiu_signals.signal import Signal`.

## Integer axes and index orderings

An [`IntegerAxis`][qiu_signals.integer_axis.IntegerAxis]`(size, ordering)` is a set of `size` samples, at least one, labeled by integer indices. The sample at array position `k` has the index `index[k]`, and the [`IndexOrdering`][qiu_signals.integer_axis.IndexOrdering] fixes which:
...
A [`PhysicalAxis`][qiu_signals.physical_axis.PhysicalAxis]`(size, period, ordering, domain)` places an integer axis in a physical domain, with the spacing `period` between neighboring samples, positive and finite. The sample at array position `k` lies at
...
The [`AxisDomain`][qiu_signals.physical_axis.AxisDomain] is `POSITION` or one of its Fourier conjugates `MOMENTUM`, `ANGULAR_WAVENUMBER` and `SPATIAL_FREQUENCY`; `is_fourier_domain` tells them apart. The subclasses fix the domain and name the spacing after it:

| class                                                                         | domain               | spacing argument |
| ----------------------------------------------------------------------------- | -------------------- | ---------------- |
| [`PositionAxis`][qiu_signals.physical_axis.PositionAxis]                   | `POSITION`           | `delta_x`        |
| [`MomentumAxis`][qiu_signals.physical_axis.MomentumAxis]                   | `MOMENTUM`           | `delta_p`        |
| [`AngularWavenumberAxis`][qiu_signals.physical_axis.AngularWavenumberAxis] | `ANGULAR_WAVENUMBER` | `delta_k`        |
| [`SpatialFrequencyAxis`][qiu_signals.physical_axis.SpatialFrequencyAxis]   | `SPATIAL_FREQUENCY`  | `delta_f`        |
...
The discrete Fourier transform of `N` samples spaced by `delta_x` samples the conjugate domain with the spacing given by [`reciprocal_period`][qiu_signals.physical_axis.reciprocal_period]:
...
The classmethods `from_position_axis` of the conjugate axis classes create the axis conjugate to a position axis, of the same size with this spacing. `hbar` is a required argument of [`MomentumAxis.from_position_axis`][qiu_signals.physical_axis.MomentumAxis.from_position_axis], and must be given for the momentum domain and only for it in `reciprocal_period`, so that the units are always explicit, e.g. `scipy.constants.hbar` for SI units or `1.0` for natural units.
...
- [`to_signal()`][qiu_signals.algebraic_signal.AlgebraicSignal.to_signal] returns the sampled [`Signal`][qiu_signals.signal.Signal] on the same axis, e.g. for its `normalized_data`.
...
[`AlgebraicSignal.from_sympy(axis, expression, symbol=None)`][qiu_signals.algebraic_signal.AlgebraicSignal.from_sympy] creates an algebraic signal from a SymPy expression, or from a string SymPy parses, compiled to a NumPy function by `sympy.lambdify`. It keeps the `expression` and the `symbol` standing for the axis values, so that the signal can be inspected and manipulated symbolically, e.g. differentiated. Signals created otherwise have `expression` and `symbol` set to `None`. It needs SymPy, e.g. via the extra `qiu-signals[sympy]`; without it, `from_sympy` raises an `ImportError` naming the extra.
...
[`SampledSignal`][qiu_signals.algebraic_signal.SampledSignal] is the type `Signal | AlgebraicSignal`, for code that only needs the `axis` and the sampled values `data`, which both kinds provide. `isinstance(value, SampledSignal)` works as well, since it is a union of classes.

```python
import numpy as np
from qiu_signals.algebraic_signal import AlgebraicSignal, SampledSignal
from qiu_signals.integer_axis import IndexOrdering
from qiu_signals.physical_axis import PositionAxis
from qiu_signals.signal import Signal
