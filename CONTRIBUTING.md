...
- Each package's documentation has a home page (`docs/index.md`: overview, installation, quick start), a user guide (`docs/user-guide.md`: concepts, conventions, pitfalls), examples (`docs/examples.md`: worked use cases) and the API reference, generated by `docs/gen_pages.py` with a page per public module, which mkdocstrings renders from the docstrings. Everything public is documented there, so keep the docstrings complete.
...
- The documentation of all packages is one site, configured by the root `mkdocs.yml`: `docs/gen_pages.py` collects the `docs/` of each package into a section of its own, named as its directory, and writes the navigation, whose sections and order are those of its `SECTIONS`. Build it with `uv run python docs/build_all.py`, which builds strictly, so broken links, anchors and references fail, and serve it with `uv run mkdocs serve`. Each package's `mkdocs.yml` previews its documentation alone, e.g. `uv run mkdocs serve -f packages/qiu-signals/mkdocs.yml`; both inherit the shared configuration `docs/mkdocs.base.yml`.
- Links between packages in their documentation point to the Markdown files of the other package in the site, from any page, e.g. `../qiu-signals/index.md` or `../qiu-qiskit-encore/user-guide.md#synthesis-methods`, so that MkDocs checks them and their anchors. The objects of the API, also of other packages, are linked by their identifiers, e.g. `` [`Signal`][qiu_signals.signal.Signal] ``. Links in READMEs are absolute GitHub URLs, so that they also work on PyPI.
...
3. Add it to the root `README.md`, to the landing page `docs/landing/index.md`, and to the `SECTIONS` of the navigation in `docs/gen_pages.py`.
