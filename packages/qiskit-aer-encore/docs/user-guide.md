...
The package consists of the module [`simulator`][qiskit_aer_encore.simulator]: the enum [`AerDevice`][qiskit_aer_encore.simulator.AerDevice] of the devices, the detection of the available devices by [`available_aer_devices`][qiskit_aer_encore.simulator.available_aer_devices], the default options of GPU simulators [`GPU_OPTIONS`][qiskit_aer_encore.simulator.GPU_OPTIONS], and the factory [`aer_simulator`][qiskit_aer_encore.simulator.aer_simulator] combining them.

## Devices

An [`AerDevice`][qiskit_aer_encore.simulator.AerDevice] selects where a simulator runs:
...
`AerDevice` is an `ExtendedEnum` of [python-encore](../../python-encore/): its members compare equal to their raw values, and [`aer_simulator`][qiskit_aer_encore.simulator.aer_simulator] accepts either. The raw values are lowercase; other strings, including Aer's own uppercase device names such as `"CPU"`, raise a `ValueError`.

```python
from qiskit_aer_encore.simulator import AerDevice, aer_simulator
...
[`aer_simulator(device, **options)`][qiskit_aer_encore.simulator.aer_simulator] returns an `AerSimulator` with its `device` option set to `"CPU"` or `"GPU"`:
...
Simulators on GPUs start from the options of [`GPU_OPTIONS`][qiskit_aer_encore.simulator.GPU_OPTIONS]:
...
The simulators run circuits of the instructions Aer supports, so transpile circuits for the simulator before running them, especially circuits with high-level gates, such as the `StatePreparation` and `QFTGate` of the `GATE` method of [qiskit-encore](../../qiskit-encore/):
