...
| module                                                        | contents                                                                                     |
| ------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| [`mps`][qiskit_mps_initializer.mps]                           | the MPS of bond dimension 2 of a state, and the layer of gates preparing it exactly           |
| [`state_preparation`][qiskit_mps_initializer.state_preparation] | the approximate preparation of any state with several such layers, and its error            |

Most users only need [`mps_state_preparation`][qiskit_mps_initializer.state_preparation.mps_state_preparation]; the functions of `mps` are its building blocks, public for inspection and for custom constructions.
...
[`bond2_mps_approximation(state)`][qiskit_mps_initializer.mps.bond2_mps_approximation] truncates a state of 2 qubits or more to a `quimb` `MatrixProductState` of bond dimension at most 2:

- The state is validated by `validated_statevector` of [qiskit-encore](../../qiskit-encore/): a Qiskit `Statevector` or its amplitudes, normalized, of a power-of-2 dimension. Single-qubit states raise a `ValueError`, since they have no MPS of 2 sites or more.
...
[`disentangler_matrices(mps)`][qiskit_mps_initializer.mps.disentangler_matrices] completes the tensors of the MPS to unitary matrices:
...
[`mps_layer(mps)`][qiskit_mps_initializer.mps.mps_layer] places these unitaries into a circuit `U` on one qubit per site, with `U|0...0>` the state of the MPS:
...
from qiskit_mps_initializer.mps import bond2_mps_approximation, mps_layer
...
[`mps_state_preparation(state, max_layers, tolerance=None)`][qiskit_mps_initializer.state_preparation.mps_state_preparation] prepares any state of at least one qubit with at most `max_layers` layers, greedily:
...
The result is an immutable [`MPSStatePreparation`][qiskit_mps_initializer.state_preparation.MPSStatePreparation]:
...
- single-qubit states, prepared by one single-qubit unitary, the Householder reflection `state_preparation_unitary` of qiskit-encore,
...
To load a real signal `f` of one sign, e.g. an intensity profile of [python-signals](../../python-signals/), into the amplitudes of a register, split it into its sum `alpha` and the state `|psi> = sqrt(f / alpha)`, such that `f = alpha |psi|**2`, with `sample_based_decomposition` of [qiskit-phase-propagator](../../qiskit-phase-propagator/), and prepare `|psi>`:
...
| `helpers.mps_technique.G_matrices`                                 | [`mps.disentangler_matrices`][qiskit_mps_initializer.mps.disentangler_matrices]               |
| `helpers.mps_technique.multi_layered_circuit_for_non_approximated` | [`mps_state_preparation`][qiskit_mps_initializer.state_preparation.mps_state_preparation]     |
...
- The package no longer depends on `pydantic`, `pydantic-numpy` and `qiskit-aer`; simulate the circuits with `Statevector(circuit)` or with [qiskit-aer-encore](../../qiskit-aer-encore/).
