...
Shared helpers for the unit tests of this monorepo that need no quantum computing: the floating-point comparisons of numbers and arrays, and the [Hypothesis](https://hypothesis.readthedocs.io/) strategies of numbers, axes and signals. It only depends on NumPy, Hypothesis and [`python-signals`](../python-signals/).
...
The package is used by the tests of `python-signals`, `python-wave-optics` and the quantum packages, and [`qiskit-pytest-helper`](../qiskit-pytest-helper/) builds its quantum strategies and assertions on it. It is a development package of the workspace, not meant to be published.
...
A property-based test of the `FFT` index ordering of `python-signals` against NumPy, on integer axes of 1 to 64 samples:
