...
All notable changes to `qiu-mps-initializer` are documented in this file. The format follows [Keep a Changelog](https://keepachangelog.com/en/1.1.0/), and the package adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html); before 1.0.0, minor versions may break the API.

## [Unreleased]

## [0.3.0]

The package moved into the [qiu](https://github.com/BlackWild/qiu) monorepo, onto its packages, and was renamed from `qiskit-mps-initializer` to `qiu-mps-initializer`; its releases up to 0.2.4 are on PyPI as [`qiskit-mps-initializer`](https://pypi.org/project/qiskit-mps-initializer/).

### Changed

- The package is installed as `qiu-mps-initializer` and imported as `qiu_mps_initializer`, formerly `qiskit_mps_initializer`.
...
- `QuantumState` is replaced by Qiskit's `Statevector`, validated by `qiu-qiskit-encore`; `QuantumIntensity` by `qiu_quantum_computing.phase_propagator.sample_based.sample_based_decomposition`; the sampling helpers by `qiu-signals`.
...
[Unreleased]: https://github.com/BlackWild/qiu/commits/master/packages/qiu-mps-initializer
[0.3.0]: https://github.com/BlackWild/qiu/tree/master/packages/qiu-mps-initializer
