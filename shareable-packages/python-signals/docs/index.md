...
`python-signals` provides uniformly sampled axes and signals on them, for any kind of numerical application, classical or quantum. An axis fixes the samples: their number, their spacing, the physical domain they live in (position or one of its Fourier conjugates) and the ordering of their integer indices. A signal is a function on such an axis, given either by its sampled values or by an algebraic expression, and supports elementwise arithmetic. It depends on NumPy and [python-encore](../python-encore/), and optionally on SymPy for symbolic expressions.

In this monorepo, the signals are the common language of the classical and the quantum numerics: the quantum packages, e.g. [qiskit-phase-propagator](../qiskit-phase-propagator/) and [qiskit-hamiltonian-simulation](../qiskit-hamiltonian-simulation/), encode them in qubit registers, the `2**n` samples of an axis being the basis states of `n` qubits whose bits encode the integer indices, and [python-wave-optics](../python-wave-optics/) builds its optical elements on them.

## Installation

```sh
pip install python-signals
pip install "python-signals[sympy]"  # with SymPy, for AlgebraicSignal.from_sympy
```
...
- The [API Reference](reference/python_signals/index.md) documents every public object, generated from the docstrings.
