...
A signal is represented by `n` qubits if its axis has `2**n` samples. [`qubit_sizes`][qiskit_pytest_helper.hypothesis_strategies.qubit_sizes] generates these sizes, and [`qubit_axes`][qiskit_pytest_helper.hypothesis_strategies.qubit_axes] axes of them in a domain of `qiu_signals.physical_axis.AxisDomain`:
...
```sh
CI=true uv run pytest packages/qiu-quantum-computing
```
